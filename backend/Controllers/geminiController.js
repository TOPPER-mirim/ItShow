...
const geminiController = {
  rewritingSentence: async (req, res) => {
    try {
      ...
      if (!userId || !filter) {
        return res.status(400).json({ error: "아이디 또는 필터가 입력되지 않습니다." });
      }
  
      const content = await models.User.findByPk(userId, {attributes: ['content'] });
      if (!content) {
        return res.status(400).json({ error: "사용자가 쓴 글이 없습니다." });
      }
